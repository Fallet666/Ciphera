...
namespace NCiphera::NCommands {
    ...
    int RunBackup() {
        ...
        NCiphera::NUtils::SudoRun(
            "sh -c 'docker exec matrix-postgres pg_dump -U synapse synapse > " + backupPath + "/synapse.sql'"
        );

        std::cout << "\n[2/3] Archiving configs and media..." << std::endl;
        NCiphera::NUtils::SudoRun(
            "sh -c 'tar -czf " + backupPath + "/synapse-config-media.tar.gz "
            "-C " + baseDir + " "
        ...
        NCiphera::NUtils::SudoRun(
            "sh -c 'tar -czf " + backupDir + "/" + backupName + ".tar.gz "
